...
* If `XRView` data is affected by a user-configured interpupillary distance, then it is strongly recommended that the UA required [explicit consent](#explicit-consent) prior to the creation of the `XRReferenceSpace` passed into `XRFrame.getViewerPose()`.
